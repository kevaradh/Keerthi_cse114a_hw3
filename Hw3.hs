...
simpleEval :: Expr -> Int
simpleEval (NumE n)            = n
simpleEval (PlusE m n)       = simpleEval m + simpleEval n
simpleEval (MinusE m n)      = simpleEval m - simpleEval n
simpleEval (TimesE m n)      = simpleEval m * simpleEval n
...
lookupVar :: String -> ListEnv -> Maybe Int
lookupVar _  []                    = Nothing
lookupVar var ((name, val):rest)
  | var == name                    = Just val
  | otherwise                      = lookupVar var rest
...
varExprListEval env (Var v)             = lookupVar v env
varExprListEval env (PlusVE m n)        = opMaybe (+) (varExprListEval env m) (varExprListEval env n)
varExprListEval env (MinusVE m n)       = opMaybe (-) (varExprListEval env m) (varExprListEval env n)
varExprListEval env (TimesVE m n)       = opMaybe (*) (varExprListEval env m) (varExprListEval env n)
...
varExprFunEval env (Var v)             = env v
varExprFunEval env (PlusVE m n)        = opMaybe (+) (varExprFunEval env m) (varExprFunEval env n)
varExprFunEval env (MinusVE m n)       = opMaybe (-) (varExprFunEval env m) (varExprFunEval env n)
varExprFunEval env (TimesVE m n)       = opMaybe (*) (varExprFunEval env m) (varExprFunEval env n)
...
instance Show VarExpr where
  ...
  show (NumVE n)           = show n
  show (Var v)             = v
  show (PlusVE m n)       = "(" ++ show m ++ " + " ++ show n ++ ")"
  show (MinusVE m n)       = "(" ++ show m ++ " - " ++ show n ++ ")"
  show (TimesVE m n)       = "(" ++ show m ++ " * " ++ show n ++ ")"
...
instance Env ListEnv where
  ...
  lookupInEnv var ((name, val): rest)
    | var == name                      = Just val
    | otherwise                        = lookupInEnv var rest
...
varExprEval env (Var v)             = lookupInEnv v env
varExprEval env (PlusVE m n)        = opMaybe (+) (varExprEval env m) (varExprEval env n)
varExprEval env (MinusVE m n)        = opMaybe (-) (varExprEval env m) (varExprEval env n)
varExprEval env (TimesVE m n)        = opMaybe (*) (varExprEval env m) (varExprEval env n)
...
evalAll env exprs = map f exprs
  where f expr = varExprEval env expr 
...
sumEval env exprs = foldr f (Just 0) exprs
  where f expr acc = opMaybe (+) (varExprEval env expr) acc
